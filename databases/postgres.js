  ...
  create: (_options = {}) => {
    ...
    return {
      ...
      get: async function () {
        ...
        if (options.debug) {
          console.log(
            `Postgres pool created [ ${options.db.user}@${options.db.host}:${options.db.port} SSL: ${!!options.db
              .ssl} db: ${options.db.database} ]`
          );
        }
  ...
  get: (model, _options = {}) => {
    ...
    const db = {
      ...
      _index_sql_queries: function () {
        ...
        return columns_to_index.map(
          (key) =>
            `CREATE INDEX ${
              options.concurrent_indexing ? 'CONCURRENTLY' : ''
            } IF NOT EXISTS ${`${options.table}_${key}_index`} ON ${options.table} ( ${key} );`
        );
      },
      ...
      where: async function (_options = {}) {
        ...
        const ordering =
          find_options.order.column !== null
            ? `ORDER BY ${
                Array.isArray(find_options.order.column)
                  ? options.column_name(find_options.order.column)
                  : find_options.order.column
              } ${find_options.order.sort}`
            : '';
      ...
      put: async function (object) {
        ...
        const on_conflict_statements = sorted_fields
          .map((field, index) => (field === options.primary_key ? '' : `${field} = $${index + 1}`))
      ...
      all: async function (criteria, _options = {}) {
        ...
        const ordering =
          query_options.order.column !== null
            ? `ORDER BY ${
                Array.isArray(query_options.order.column)
                  ? options.column_name(query_options.order.column)
                  : query_options.order.column
              } ${query_options.order.sort}`
            : '';
